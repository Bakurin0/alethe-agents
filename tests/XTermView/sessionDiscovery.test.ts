...
test('concurrent panes cannot claim the same discovered session', () => {
  ...
  assert.equal(claimDiscoveredSession('codex', 'C:\\repo', before, sessions)?.id, 'new')
  assert.equal(claimDiscoveredSession('codex', 'C:\\repo', before, sessions), undefined)
})

// Se duas sessões novas aparecem entre snapshots, a associação pane -> conversa é
// ambígua: melhor não reivindicar nenhuma do que retomar o chat errado no próximo boot.
test('ambiguous new sessions are never claimed', () => {
  resetSessionClaimsForTests()
  const before = new Set(['old'])
  const sessions = [
