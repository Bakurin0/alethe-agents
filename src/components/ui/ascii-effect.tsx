...
function parseHex(color: string): [number, number, number] | null {
  ...
  if (short) return short.slice(1).map((part) => Number.parseInt(`${part}${part}`, 16)) as [number, number, number]
  const full = /^#([\da-f]{2})([\da-f]{2})([\da-f]{2})$/i.exec(color)
  return full ? full.slice(1).map((part) => Number.parseInt(part, 16)) as [number, number, number] : null
}
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      for (let index = 0; index < luminance.length; index += 1) {
        ...
        let value = (pixels[pixel] * 0.2126 + pixels[pixel + 1] * 0.7152 + pixels[pixel + 2] * 0.0722) / 255
        value = clamp((value - 0.5) * Math.max(0, contrast) + 0.5)
        value = clamp(value * brightnessBoost * alpha)
        luminance[index] = value <= threshold ? 0 : (value - threshold) / Math.max(0.001, 1 - threshold)
      }
      ...
      if (dither === 'bayer') {
        ...
        luminance.forEach((value, index) => {
          ...
          luminance[index] = clamp(value + (matrix[(row % 4) * 4 + col % 4] / 16 - 0.5) * clamp(ditherStrength) / 4)
        })
      } else {
        luminance.forEach((value, index) => {
          const quantized = Math.round(clamp(value) * (steps - 1)) / (steps - 1)
          luminance[index] = dither === 'floyd-steinberg'
      ...
      for (let row = 0; row < rows; row += 1) {
        for (let column = 0; column < columns; column += 1) {
          ...
          if (variant === 'flow' && !reduceMotion) {
            ...
            if (pointer.current.active && mouseRadius > 0) {
              ...
              if (distance > 0 && influence > 0) {
                ...
                sourceColumn -= mouseX / distance * influence ** 2 * mouseStrength * ripple / cellWidth
                sourceRow -= mouseY / distance * influence ** 2 * mouseStrength * ripple / cellHeight
              }
            }
          }
          ...
          context.fillText(character, column * cellWidth - cellWidth + (glitchBands.get(row) ?? 0), row * cellHeight - cellHeight)
        }
      }
    }
